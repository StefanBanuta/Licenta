...
int main() {
    ...
    ResNet network(num_classes, false);


    const std::filesystem::path checkpoints_dir("../checkpoints/ResNet_aug_1e-3");
    ...
    const int total_iters = 6000;   
    const float learning_rate = 1e-3; 
    const int valid_inters = 1000;    
    const int save_iters = 2000;      
    float mean_loss = 0.f;            
    float cur_iter = 0;               
